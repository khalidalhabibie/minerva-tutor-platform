    ...
    <Suspense
      fallback={
        <main className="mx-auto max-w-6xl px-4 py-6">
          <LoadingState label="Loading invited cases" />
        </main>
      }
    >
      <TutorCasesContent />
...
function TutorCasesContent() {
  ...
  function updateFilter(key: string, value: string) {
    ...
    router.push(`/tutor/cases?${next.toString()}`);
  }
    ...
    <AuthGate allowedRoles={["TUTOR"]}>
      <AppShell>
        ...
        <p className="mt-1 text-sm text-slate-600">
          View cases where a parent has invited you.
        </p>
          ...
          <ApiResult error={cases.error} isLoading={cases.isLoading}>
            {cases.data && cases.data.data.length === 0 ? (
              <EmptyState
                ...
                detail="Cases will appear here after a parent invites you."
              />
            ) : (
              <div className="overflow-hidden rounded-md border border-line bg-white">
                <div className="divide-y divide-line">
                  {cases.data?.data.map((tuitionCase) => (
                    <Link
                      ...
                      href={`/tutor/cases/${tuitionCase.id}`}
                      key={tuitionCase.id}
                    >
                ...
                {cases.data ? (
                  <div className="flex items-center justify-between border-t border-line px-4 py-3 text-sm text-slate-600">
                    <span>
                      Page {cases.data.pagination.page} of{" "}
                      {Math.max(cases.data.pagination.totalPages, 1)}
                    </span>
                    <div className="flex gap-2">
                      ...
                      <button
                        className="rounded-md border border-line px-3 py-1.5 disabled:opacity-50"
                        disabled={page >= cases.data.pagination.totalPages}
                        onClick={() => goToPage(page + 1)}
                        type="button"
                      >
                        Next
